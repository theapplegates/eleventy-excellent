...
<script>
  (() => {
    ...
    const copyStatus = document.getElementById('pgp-copy-status');
    const keyBlock = document.getElementById('pgp-public-key');

    if (!copyButton || !copyStatus || !keyBlock) return;

    async function copyPGPKey() {
      const text = keyBlock.innerText.trim();
      const originalLabel = copyButton.textContent;

      try {
        await navigator.clipboard.writeText(text);
        copyButton.textContent = 'Copied';
        copyStatus.textContent = 'Public key copied to clipboard.';
      } catch (error) {
        copyButton.textContent = 'Copy failed';
        copyStatus.textContent = 'Clipboard copy failed. You can still select and copy manually.';
      }
    ...
    copyButton.addEventListener('click', copyPGPKey);
  })();
</script>
